...
# exception handling
@app.exception_handler(500) # error 500 handling
    ...
    if username == "" or password == "":
      ...
      if Authentication.auth_login(username.value, password.value):
        ...
        if config['debug']['debug_mode']:
          ui.notify('login successful') # not showing
          Debug.info('Login Successful')
          Debug.info(f"Stored Username: {app.storage.user.get('username')}")
          Debug.info(f"Authenticated: {app.storage.user.get('authenticated')}")
        else:
          # ui.notify('login failed') # not triggering
          pass
...
# page ui
  ...
  app.storage.user.indent = True # helps make save files easier to read
...
# console start up message
  ...
  if config['debug']['flags']['ignore_delays']:
    Debug.info(f'Bypass Delays       | {start_up_color['flag enabled']}True')
  else:
    Debug.info(f'Bypass Delays       | {start_up_color['flag disabled']}False')
  print('')
